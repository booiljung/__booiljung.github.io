...
통신 매체의 양 종단에는 송신자와 수신자가 있습니다. 송신자는 정보를 보내는 종단, 수신자는 정보를 받는 종단입니다.
...
인간이 일반적인 대화 할때 양방이 동시에 말하며 듣지 않습니다. 앨리스가 말하면 밥은 듣기만하고, 밥이 말하면 앨리스는 듣기만 합니다. 이것을 반이중통신이라고 합니다.

아래 그림을 보면 통신 미디어는 1개인데 송신기와 수신기가 하나의 통신 미디어에 장착되어 있습니다.

![1565581461100](communication_basics.assets/1565581461100.png)

통신 미디어 또는 채널이 1개이므로 어느 일방만이 정보를 전송하고, 어느 일방은 받을 수만 있습니다. 아래 그림을 보면 앨리스가 일방적으로 밥에게 정보를 보내고, 밥은 받기만 하고 있습니다. 밥은 정보를 보낼 수 없습니다.

![1565581421366](communication_basics.assets/1565581421366.png)

밥이 정보를 보내면 이번에는 앨리스가 받기만 합니다.

![1565581545965](communication_basics.assets/1565581545965.png)

전이중통신은 양방이 동시에 정보를 보내고 받을 수 있어야 하므로 더 좋은 통신 방식이 아닌가 싶은데,  2개의 미디어나 채널을 설치해야 합니다. 짧은 거리라면 비용이 많이 들지 않지만, 수백킬로의 거리에 선로를 설치해야 한다면 많은 비용이 들게 됩니다. 반이중통신은 양방이 송신을 할 수 있되, 어느 한순간만은 일방만이 미디어를 통해 데이터를 송신할 수 있으므로 미디어 설치 비용을 아낄 수 있습니다.

반이중통신은 어느 한 순간에 하나의 단말기만 송신할 수 있지만, 규약에 따라 고속으로 빠르게 송신할 수 있는 기술이 적용된다면 마치 전이중처럼 보이게 됩니다. 현대의 많은 통신들이 빠른 반이중 통신을 통해 전이중 통신처럼 사용할 수 있게 합니다. 대표적으로 사무실의 LAN에서 사용하는 이더넷이 반이중통신이며 빠른 속도로 전이중통신처럼 보이게 합니다.
...
![1565582281839](communication_basics.assets/1565582281839.png)[그림] OSI 참조 모형
...
물리 계층은  전기적, 기계적 특성을 사용하여 신호를 전송합니다. 이 계층의 전송단위는 신호로 보통 0이나 1인 비트를 나타냅니다. 어느 미디어의 경우는 복수의 비트를 보낼 수도 있습니다.

이 계층에는 RS-232, RS-422, V-35, 10Base-T 등이 있습니다.

쉽게 예를 들면 네트워크에 연결된 리피터, 전기적 또는 전자기파 송수신 장치 등을 말합니다.
...
이 계층의 전송단위는 프레임(frame)이며 이 프레임에는 오류 검출를 위한 구조를 가집니다.
...
NetBEUI, IP, ARP가 이 계층에 있습니다. 여기서 IP는 TCP/IP의 그 IP를 말합니다. IP 주소가 이 계층에 해당합니다. 어느 자료를 보면 ARP를 계층 2로 소개하는 경우가 있는데, ARP가 IP를 부여하는 역할을 하는데, 계층2라면 IP를 알 수 없으며, IP와 MAC 주소를 알고 이것을 관리하므로 계층 3가 맞습니다. 

쉽게 예를 들면, 중계 노드는 인터넷 공유기가 있습니다. 인터넷 공유기를 라우터 또는 게이트웨이라고도 합니다. 인터넷 공유기는 사무실 내부망과 외부망인 인터넷을 연결하는 역할을 합니다. 우리가 컴퓨터를 사용하여 어느 경우에는 내부의 다른 컴퓨터의 파일을 주고 받고, 어느 경우에는 외부 인터넷의 파일을 주고 받습니다. 이 과정에는 컴퓨터와 각 노드들이 어디로 데이터를 보내야 할지 알고 있으며 네트워크 계층이 이 구별을 담당합니다. 계층 3의 전문적인 장비는 L3 스위치라고도 합니다.

![1565582637652](communication_basics.assets/1565582637652.png)

##### 계층 4: 전송 계층 (transport layer; 트렌스포트 레이어)

전송 계층은 종단간(end to end)의 신뢰성 있는 데이터를 주고 받을 수 있도록 하여, 상위 계층들이 데이터 전달에 있어 유효성이나 효율성을 생각하지 않도록 합니다. 일부 프로토콜은 상태 개념이 있으며, 연결 지향입니다. 이것은 패킷들의 전송이 유효한지 확인하고 전송에 실패한 패킷들은 재전송하는 것을 말합니다. 대표적으로 TCP가 있습니다. TCP는 TCP/IP를 말할때 그 TCP입니다.
...
응용 레이어는 L7 입니다. 웹브라우저가 웹서버에 요청을 보내면 그 요청이 네트워크에 올려지기까지  캡슐화를 하고, 서버에서 수신하면 디캡슐화 과정을 거칩니다.

## 랜과 인터넷 (LAN and Internet)

비엔지니어의 경우 랜을 랜케이블 정도로, 그리고 나머지 컴퓨터 네트워크를 인터넷으로 이해하고 있습니다. 이 이해가 엔지니어와 비엔지니어 사이의 의사 소통이 어렵게 하기도 합니다. 이 글은 LAN과 인터넷을 엄격하게 구분하여 사용합니다.
...
WAN은 Wide Area Network의 약자로 광대역 네트워크를 말합니다. WAN의 대표적인 예로 인터넷이 있으며, 가정이나 사무실에서 사무실 밖으로 연결하려면 인터넷 공유기를 통해서 연결 되어야 합니다. 인터넷 공유기를 라우터라고도 합니다. KT나 SKT같은 기간통신사업자들 간에 데이터를 전송하는데도 라우터가 사용됩니다.
...
앨리스가 사무실에서 같은 사무실의 밥에게 파일을 전송하려면, 앨리스 컴퓨터에 달려있는 NIC, 스위칭 허브들, 그리고 밥의 컴퓨터에 NIC만을 통하여 파일을 보낼 수 있습니다.

앨리스가 다른 회사의 캐롤에게 파일을 전송하려면 WAN을 통해서 파일을 보내야 합니다. 이때 앨리스가 보내는 파일은 앨리스의 컴퓨터에 있는 NIC, 스위칭 허브, 라우터 (인터넷 공유기)를 거쳐서 기간통신사업자들의 라우터들을 거친 다음, 캐롤의 사무실에 있는 라우터, 스위칭허브들을 거쳐 캐롤의 컴퓨터에 장착된 NIC를 거치고, 비로소 캐롤이 파일을 받게 됩니다.

#### NIC

Network Interface Controller의 약자로 컴퓨터를 네트워크에 연결하기 위해 사용됩니다. 네트워크 카드, 랜카드, 네트워크 인터페이스 카드, 네트워크 어댑터, 이더넷 카드 등 다양한 이름으로도 불립니다. 

NIC는 OSI 7계층에서 계층1 물리 계층과 계층 2 데이터링크 계층을 담당합니다. NIC는 계층2를 담당하므로 제조사에서 출시 될때 MAC 주소가 부여 되어 있습니다.
...
이더넷의 스펠링을 보면 ethernet으로 ether - net 입니다. ether는 그리스어 에테르(ether)입니다. 에테르는 매질이라는 뜻으로 고체를 녹일때 액체를 말하기도 하며, 빛을 전달하는 매질의 의미로도 사용되었습니다. 그러나 이 에테르라는 이름은 "전자기파 전파를 위한 편재하는 완전 수동적 매체"로 존재할 것으로 예언된 루미 페루 에테르의 이름을 따서 명명하였습니다.
...
쉽게 비유하자면 1차선의 복잡한 교차로라고 할 수 있습니다. 교차로에는 동시에 한방향의 차량만 통과 할 수 있습니다. 교차로를 여러 차량이 동시에 통과하려면 충돌하여 사고가 발생합니다. 교차로를 통과하기 위해서는 지켜 보고 있다가, 다른 차량이 통과하지 않으면 통과해야 합니다. 물론 출발할때 다른 차량도 출발하지 않는지 눈치를 봐야 합니다. 
...
이더넷 허브는 단순한 루프 구조라서 단자에 프레임을 전송 합니다.

![1565593407546](communication_basics.assets/1565593407546.png)

#### 이더넷 스위치 (ethernet switch)

일반적으로 스위칭 허브라고 불리는 이더넷 스위치입니다. 전문 시스템을 위해 사용하는 장비는 L2 스위치라고 합니다. L2 스위치에서 L2는 OSI 7계층의 계층 2를 말합니다.

앞에서 이더넷에 연결된 모든 장치는 MAC 주소를 가지고 있다고 하였습니다. 그리고, 이더넷 프레임은 송신자 MAC 주소와 수신자 MAC 주소를 포함하고 있다고 하였습니다. 그래서, 중간에서 프레임을 중계하는 장치는 어느 단자에 어느 MAC 주소를 가진 장치가 있는지 알고 이것을 메모리에 기억할 수 있습니다. 
...
MAC A가 MAC D로 프레임을 보내겠다고 보냅니다. 

![1565593853663](communication_basics.assets/1565593853663.png)

이때, 단자 1에 MAC A가 보낸 프레임이 들어 왔으므로, 단자 1에 MAC A가 있다고 기억해 둡니다. 이더넷 스위치가 어느 단자에 MAC 주소가 있다는 것을 암기하는 과정을 학습 (Learning)이라고 합니다.

![1565594013382](communication_basics.assets/1565594013382.png)

MAC A는 프레임을 MAC D에 보내고자 하고, 이더넷 스위치는 아직 MAC D가 어느곳에 있는지 알지 못합니다. 모르면 전체 단자에 프레임을 뿌립니다. 목표 MAC 주소를 모를때 전체 단자에 뿌리는 과정을 플루딩(Flooding)이라고 합니다.

![1565594089340](communication_basics.assets/1565594089340.png)

프레임을 받은 MAC B, C는 그 프레임이 자신에게 온것이 아니기 때문에 무시합니다. MAC D는 자신에게 온것임을 알기때문에 그것을 처리 할 것이고, 필요에 따라 다시 MAC A에게 응답을 할 것입니다. 

![1565594227365](communication_basics.assets/1565594227365.png)

MAC D로 부터 프레임을 받은 이더넷 스위치는 단자 4에 MAC D가 있다는 것을 알고 기억하게 됩니다.

![1565594305254](communication_basics.assets/1565594305254.png)

이제 이더넷 스위치는 MAC A가 단자 1에 연결되어 있다는 것을 알고 있습니다. 따라서, 전체 단자에 프레임을 뿌릴 필요 없이 단자 1에만 프레임을 보냅니다.
...
이더넷 스위치는 주어진 단자로만 프레임을 보내므로, 프레임이 충돌할 가능성이 낮으며, 이더넷 장치에 고장이 발생하더라도 악영향을 줄일 수 있습니다. 하지만, 이더넷 스위치를 사용하더라도 대규모 네트워크의 경우 가장 상위에 있는 이더넷 스위치에 부하가 높아져 처리를 하지 못할 수도 있습니다. 대규모 네트워크의 경우에는 라우터,  가상랜을 사용하여 네트워크를 분리해야 합니다. 그림에서 가장 위에 있는 이더넷 스위치는 집중적으로 프레임을 처리해야 해서 심한 경우 처리를 못할 수도 있습니다. 이경우 네트워크를 분리해야 하며, 이것을 서브네팅(Subneting)이라고 합니다. 서브넷에 대해서는 아래에서 언급하게 됩니다.
...
NIC가 MAC 주소를 가지고 있듯이, 인터넷을 사용하는 장비는 IP 주소를 부여받게 되며, MAC 주소와 달리 IP 주소는 변경할 수 있습니다.
...
IP 주소는 IPv4와 IPv6로 나뉩니다. 일반적인 업무는 주로 IPv4로 설정하므로, 이 글은 IPv4 기준으로 설명합니다.
...
공인 IP 주소는 제한된 자원이며, 사용을 위해서는 비용을 지불해야 합니다. 따라서, 각 로컬 영역의 호스트들은 사설 IP주소를 사용하며, 호스트들이 인터넷을 통해 패킷을 보낼때 라우터들이 사설 IP를 공인 IP로 변환하여 통신을 하게 됩니다.

사설 IP 주소는 인터넷에서는 보이지 않으며 공인 IP로 보이게 됩니다. 만일 같은 로컬 네트워크를 사용하는 앨리스, 이브가 한메일에 접속하면, 한메일 서버에서는 앨리스와 이브를 하나의 공인 IP를 사용하는 것처럼 보이게 됩니다. 만일 이브가 한메일을 악의적으로 해를 끼칠려고 하였고, 한메일 담당자가 이브의 공인 IP주소를 차단하면, 앨리스까지 한메일을 사용할 수 없게 됩니다. 따라서 인터넷에서 서비스를 운영하는 자는 IP 주소 차단에 주의를 기울려야 합니다.

사설 IP 주소는 크게 3가지 클래스가 있습니다.

|  클래스  |            주소 범위            |      네트워크 수량       |         호스트 수량          |
| :------: | :-----------------------------: | :----------------------: | :--------------------------: |
| 클래스 A |  `10.0.0.0` ~ `10.255.255.255`  | 1개의 A클래스 네트워크,  |      전체 16,581,375개       |
| 클래스 B | `172.16.0.0` ~`172.31.255.255`  | 16개의 B 클래스 네트워크 | 네트워크당 65,536개의 호스트 |
| 클래스 C | `192.168.0.0`~`192.168.255.255` | 256개의 C클래스 네트워크 |  네트워크당 256개의 호스트   |
...
요약하면
	...
	- LAN은 L2 기준, 서브넷은 L3 기준.
	- LAN은 이더넷 사용, 서브넷은 IP (인터넷 프로토콜) 사용.

##### 서브넷 마스크

사설 IP 주소는 네트워크 ID과 호스트 ID으로 나누어 집니다. 서브넷 마스크는 사설 IP 주소를 네트워크 ID와 호스트 ID로 구분하는 역할입니다.

사설 IP 주소에서 네트워크 ID가 같으면 같은 로컬 네트워크에 묶여 있습니다. 만일 네트워크 ID가 다르면 라우터를 통해 패킷이 나가게 됩니다.

서브넷 마스크를 구성하는 방식에는 디폴트 서브넷 마스크와 서브넷 마스크가 있습니다.

###### 디폴트 서브넷 마스크

디폴트 서브넷 마스크는 사설 IP 주소의 클래스에 따라 주어진 마스크 입니다. 위에서 사설 IP 주소는 3가지 클래스가 있다고 하였습니다. 클래스 A, 클래스 B, 클래스 C 입니다.
...
클래스 A는 `10.0.0.0` 부터 `10.255.255.255` 범위의 사설 IP 주소를 사용할 수 있습니다. 디폴트 서브넷 마스크가 `255.0.0.0`이면 `10.0.0.0` 부터 `10.255.255.255` 까지 전체 IP 주소를 하나의 서브넷으로 사용할 수 있습니다. 이때 네트워크 ID는 `10` 하나이며, 호스트 아이디가 `0.0.0` 부터 `255.255.255` 까지 입니다.
...
클래스 C의 디폴트 서브넷 마스크는 `255.255.255.0`이고 이것을 2진수로 표현하면 `11111111.11111111.11111111.0000000`입니다. 각 서브넷에 최대 호스트는 30개 이므로, 32개의 호스트 ID를 부여 할 수 있습니다. 호스트 아이디는 `0`부터 `31`까지로 32개이며, 서브넷 마스크는 이진수로 `11111111.11111111.11111111.11110000` 이렇게 부여하면, 각 서브넷은 32개의 호스트 ID를 사용할 수 있습니다.
...
라우터는 OSI 7 계층에서 계층 3 네트워크 계층을 담당합니다. 패킷을 처리하며, 서브넷 또는 로컬 네트워크에서 다른 서브넷 또는 로컬 네트워크로 패킷을 전송하는 역할을 담당합니다. 라우터는 가장 빠른 경로를 알고 있으며, 빠른 경로에 있는 라우터들을 통해 패킷을 전송을 하게 됩니다.
...
자동으로 설정하는 경우는 컴퓨터에 대해 잘 모르고, 다른 컴퓨터나 장비가 이 컴퓨터에 IP 주소로 접속하지 않을때 사용할 수 있습니다.
...
만일 클라이언트의 주소가 `192.168.2.1`, 서브넷 마스크가 `255.255.255.0`이고 , 서버의 주소가 `192.168.2.10`이면 동일 서브넷에 들어 있습니다. 그런데, IP 주소는 알고 있지만, 실제 데이터 프레임은 이더넷을 통해서 보내게 됩니다. 윈도우는 IP 주소를 판별하여 이 IP 주소가 사설 IP 주소인지, 서브넷에 해당하는지 판단을 합니다. 서브넷에 해당하면 IP 주소에 MAC 주소를 찾게 됩니다. 이 주소는 운영체제 내부에 ARP 테이블이라는 영역에 저장되어 있고, 이 테이블에서 MAC 주소를 얻어 패킷을 전송하게 됩니다.

만일 서버의 주소가 `192.168.3.1`이면 같은 서브넷이 아니므로, 윈도우 클라이언트는 패킷을 라우터로 보냅니다. 라우터는 받은 패킷의 IP 주소를 분석하여, 자신이 가진 테이블에서 해당 서브넷의 라우터로 패킷을 보내게 됩니다.

만일 서버의 주소가 공인 IP 주소이면, 윈도우 클라이언트는 패킷을 라우터로 전송합니다. 패킷을 받은 라우터는 패킷에 포함된 IP 주소를 분석하여 해당 라우터로 가는 가장 빠르다고 생각되는 경로로 패킷을 보내게 됩니다.

### 도메인 이름 (도메인 네임; Domain Name)

IP 주소는 `103.104.19.5` 처럼 숫자로 이루어져 있습니다. 이 숫자는 암기하기 어렵습니다. 그리고, 공인 IP 주소는 대륙, 국가, 인터넷 망 사업자에게 할당된 것으로 지역적으로 고정되어 있다고 볼 수 있습니다. 한국에 있는 서버를 미국에 옮긴다면 공인 IP 주소를 바꿔야 할 것입니다. IP 주소를 암기해서 인터넷을 사용하는 중에 서버가 미국으로 옮겨가 IP주소가 바뀌면 고객들은 서버에 접속할 수 없을 것입니다.

암기하기 어렵고, 인터넷망사업자에게 고정된 IP 주소의 문제를 해결하기 위해 도메인 이름이 있습니다. 도메인 이름은 Domain Name입니다.

도메인 이름이란 IP 주소를 암기하기 쉬운 알파벳 및 숫자로 대치한 것을 말합니다.
...
도메인 이름을 등록 받고 관리하는 기관이 있으며, 기본적으로 1년동안 수천원에서 수십만원을 지불하고 도메인 이름을 구매합니다.

사람이 편리하게 도메인 이름만 입력하면 IP 주소로 변환하게 되는데, 컴퓨터가 서버에 접속할때 도메인 이름이면 도메인 네임 서버에 접속하여 IP 주소를 얻고, 이 IP 주소로 해당 서버에 접속 합니다. 이 동작은 사용자의 눈에 보이지 않으므로 도메인 네임 서버의 존재를 눈치채지 못할 수 있습니다. 이 도메인 네임 서버를 짧게 DNS로 줄여서 표현합니다. 이후 다시 DNS를 언급하겠습니다.

### PING

핑은 Packet INternet Groper의 약자로 인터넷 프로토콜을 통해 특정한 호스트가 다른 호스트에 도달 할 수 있는지 테스트하는 목적으로 사용되는 컴퓨터 네트워크 도구입니다. (위키피디아 인용)

이름에 Packet 이 들어 있어서 눈치를 채셨겠지만 핑은 OSI 참조모델 L3 IP에서 동작합니다.
...
응답에 보면 `TTL`이 있습니다. `TTL`은 Time To Live의 약자로, 도중에 얼마나 많은 라우터를 거쳤는지 표시합니다. 윈도우를 사용하는 호스트는 기본적으로 `128`을 보내고, 유닉스를 사용하는 호스트는 `64`를 보냅니다. 그리고 라우터를 한번 거칠때마다 라우터가 `TTL` 값에서 `1`을 뺍니다. 여기 구글 예제에서 `TTL`이 `52`이므로, 구글은 운영체제로 유닉스 계열을 사용하고 있고, 구글에서 저의 컴퓨터까지 `64-52`  즉 `12`개의 라우터를 거쳤다고 유추할 수 있습니다.

이 `TTL`이 왜 있느냐 하면, 많은 라우터를 거친 패킷은 인터넷상에 문제가 있는 것으로 간주하여 버리도록 하기 위한 것입니다.
...
ping을 보냈을때 time out이 되거나 시간이 지나치기 길면 네트워크에 문제가 있습니다. 예제로 네트워크상에 존재하지 않는 IP 주소 `192.168.3.10`에 ping을 보내 보겠습니다.
...
`인터넷 프로토콜 버전 4(TCP/IPv4) 속성`창을 보면 아래에 `도메인 네임 서버 주소`를 설정할 수 있습니다.

![1565608305749](communication_basics.assets/1565608305749.png)

앞에서 도메인 이름이 있고, 도메인 이름을 IP 주소로 바꿔주는 도메인 네임 서버; DNS가 있다고 하였습니다. 이 창에서 DNS가 도메인 네임 서버를 지정하는 것입니다. 도메인 네임 서버는 구글 같은 업체나 인터넷 망 사업자들이 지원합니다. 거리가 멀 수록 DNS까지의 도달하는데 시간이 걸려서 느릴 수 있으니 가까운 DNS를 설정하는 것이 좋습니다. 일반적으로 가입한 인터넷망 사업자가 운영하는 DNS가 낫습니다.
